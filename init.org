...
#+begin_src emacs-lisp
  (use-package python
    ...
    ;; (advice-remove 'python-shell-send-string 'maschm/python-shell-scroll-end)
  ...
  (use-package pyvenv
    :after python
    :config
    (setq pyvenv-mode-line-indicator '(pyvenv-virtual-env-name ("[P:" pyvenv-virtual-env-name "] ")))
    (pyvenv-mode))
...
(defun copy-current-defun-path-as-kill ()
  ...
  (let* ((module-path (python-file-module-path))
         (cur-def (python-info-current-defun))
         (path-with-line-number (concat module-path "." cur-def)))
