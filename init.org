...
(use-package general
  ...
  (my-menu-def
    "H ?" '(general-describe-keybindings :which-key "show bindings")
...
*** Window & Buffer keybindings menu - SPC B

#+BEGIN_SRC emacs-lisp
(my-menu-def
  "<tab>" '(mode-line-other-buffer :which-key "last-buffer")
  "TAB" '(mode-line-other-buffer :which-key "last-buffer") ;; for terminal?
  "<M-tab>" '((lambda ()
                (interactive)
...
(setopt split-window-preferred-function #'split-window-sensibly-prefer-horizontal)
...
(use-package exec-path-from-shell
  ...
  (add-to-list 'exec-path-from-shell-variables "GOOGLE_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "OPENAI_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "OPENAI_RSPROXY_TT_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "OPENAI_UNDERLYING_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "PERPLEXITY_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "MISTRAL_API_KEY")
  (add-to-list 'exec-path-from-shell-variables "JSON_BASED_PYTHON_LOGGING")
...
* Navigation & Minibuffer & Search & CAPF
** i3
https://github.com/junyi-hou/i3-mode

i3 config:
#+begin_src i3
bindsym $mod+$up exec ~/.local/bin/i3-call focus up "C-w $up"
bindsym $mod+$down exec ~/.local/bin/i3-call focus down "C-w $down"
bindsym $mod+$left exec ~/.local/bin/i3-call focus left "C-w $left"
bindsym $mod+$right exec ~/.local/bin/i3-call focus right "C-w $right"
#+end_src
...
(defun maschm/my-quit ()
  "Quit the top-most minibuffer or annoying buffer"
  (interactive)
  (cond
   ...
   ((get-buffer-window "*Help*")
    (quit-closest-window "*Help*"))
   ((get-buffer-window "*Backtrace*")
    (quit-closest-window "*Backtrace*"))
   ((get-buffer-window "*Warnings*")
    (quit-closest-window "*Warnings*"))
   ((get-buffer-window "*Diff*")
    (quit-closest-window "*Diff*"))
   ((string-match-p "^magit: " (buffer-name))
    (keyboard-quit))
   ((if-let ((buffer (get-buffer-window "*devdocs*")))
        (progn (quit-closest-window buffer) t)))
   ((if-let ((buffer (get-buffer-window-by-regex "^magit")))
        (progn (quit-closest-window buffer) t)))
   ((if-let ((buffer (get-buffer-window-by-regex "^\\*helpful")))
        (progn (quit-closest-window buffer) t)))
   ((if-let ((buffer (get-buffer-window-by-regex "^\\*info")))
        (progn (quit-closest-window buffer) t)))
   ((if-let ((buffer (get-buffer-window-by-regex "^\\*")))
        (progn (quit-closest-window buffer) t)))
   ((if-let ((buffer (get-buffer-window-by-regex "init.org")))
        (progn (quit-closest-window buffer) t)))
    ; not sure when this is triggered
   (t (keyboard-quit))))

(defun get-buffer-window-by-regex (regex)
  "Return first visible buffer whose name matches REGEX.
Visible means the buffer is currently displayed in some window."
  (cl-find-if
   (lambda (buf)
     (string-match-p regex (buffer-name buf)))
   (mapcar #'window-buffer (window-list))))

(defun quit-closest-window (buffer-or-name)
  "Quit the window displaying BUFFER-OR-NAME.
If the current window displays BUFFER-OR-NAME, delete it if there are more than one window, otherwise quit it.
Otherwise, find and quit the first window that displays the buffer."
  ;; Ensure that buffer-or-name is converted to a buffer object.
  (if-let ((buffer (get-buffer buffer-or-name)))
      ;; Check if current window is displaying the buffer.
      (if (eq (current-buffer) buffer)
          ;; Delete the current window. if there are more than one
          (if (> (count-windows) 1)
              (delete-window)
            (quit-window))
        ;; Otherwise, find and delete the first window that displays the buffer.
        (if-let ((window (get-buffer-window buffer))) ;; Get window showing the buffer
            (with-selected-window window
              (quit-window))))))
...
;; UNUSED
;; this is an atept to find temporary buffers
;; maybe improve this by giving it a time limit for how recent a buffer must be
;; then use it with different thresholds in my-quit
(defun maschm/get-latest-temp-output-buffer ()
  "Find the most recently created buffer from `with-output-to-temp-buffer`.
Returns the buffer object or nil if none found."
  (let ((newest-buffer nil)
        (newest-time 0)
        (temp-buffer-modes '(help-mode special-mode compilation-mode)))
    (dolist (buf (buffer-list))
      (with-current-buffer buf
        (when (and (memq major-mode temp-buffer-modes)
                   (or (string-prefix-p "*" (buffer-name)) ;; naja
                       (string-suffix-p "*" (buffer-name))))
          (let ((buffer-time (or (buffer-local-value 'buffer-display-time buf)
                                 (current-time))))
            (when (time-less-p newest-time buffer-time)
              (setq newest-time buffer-time)
              (setq newest-buffer buf))))))
    newest-buffer))
#+end_src
...
(use-package consult
  ...
  (general-def
    ...
    "C-x <tab>" 'mode-line-other-buffer)
...
(defun vertico-directory-delete-char (&optional n)
  ...
  (unless (and (or (eq (char-before) ?/)
                   ;; also delete the file if it fully matches candidate
                   (string= (buffer-substring-no-properties (minibuffer-prompt-end) (point)) (vertico--candidate))
                   (string= (concat (buffer-substring-no-properties (minibuffer-prompt-end) (point)) "/") (vertico--candidate)))
...
(use-package embark
  ...
  (general-def :keymaps 'embark-identifier-map
    "b" 'consult-buffer
    )

  (general-def :keymaps 'embark-buffer-map
    "p" 'pop-to-buffer)
...
(advice-add 'evil-org-> :after #'maschm/visual-restore-after-shift)
#+end_src

*** evil read only adjust insert position

#+begin_src emacs-lisp
(defun maschm/adjust-point-for-readonly (&rest _)
  (let* ((pos (point))
         (before-readonly (and (> pos (point-min))
                             (get-text-property (1- pos) 'read-only)))
         (at-readonly (get-text-property pos 'read-only))
         (after-readonly (and (< pos (point-max))
                            (get-text-property (1+ pos) 'read-only))))

    (cond
     ;; check if current pos is readonly
     (at-readonly
...
(add-hook 'evil-insert-state-entry-hook 'maschm/adjust-point-for-readonly )
#+end_src
  ...
  (transient-append-suffix 'magit-push "-F"
    `("-o" "Transmit string" ("-o" "--push-option=")
      ...
      :choices ,(map-apply (lambda (str suf)
                           ...
                           '(("skip-ci" . "SourceHut")
                             ("ci.skip" . "GitLab")
                             ;; ("merge_request.create --push-option=merge_request.auto_merge" . "MR")
                             ("merge_request.create" . "MR-c")
                             ("merge_request.auto_merge" . "MR-a")
                             ("merge_request.target=main" . "MR-m")
                             ("merge_request.squash" . "MR-s")
                             ("merge_request.draft" . "MR-d")
                             ("merge_request.remove_source_branch". "MR-x")
                           ;; (merge_request.title=". "MR-t")
                           ))))
  )
#+END_SRC
...
[[info:emacs#Fonts]] [[info:elisp#Fonts]]
...
text-mode hook was to much why is e.g. yaml derived from text mode...
...
"
    ...
    ("C--" default-text-scale-decrease nil)
    ("=" default-text-scale-reset nil)
    ("i" text-scale-increase nil)
    ("C-i" text-scale-increase nil)
...
(use-package jinx
  ...
  (general-def :keymaps 'global
    "M-$" 'jinx-correct-word
    "C-M-$" #'jinx-languages)
  (general-nmap "z =" 'jinx-correct-word)
  :custom
  (jinx-languages "en_US-large de_DE")
  ;; looks like uk_UA hides English errors
  ;; (jinx-languages "en_US-large de_DE uk_UA")
  )
...
(use-package copilot
  ...
  :commands (copilot-diagnose copilot-login)
  )
#+end_src
...
** special-mode
enable "q" to burry-window in special-mode
...
(use-package helpful
  ...
  :local
  (emacs-startup-hook . ((require 'info-look)
                         ;; pay initial cost early
                        (info-lookup-setup-mode 'symbol 'emacs-lisp-mode)
                        ))
  (helpful-mode-hook . ((setq-local eval-expression-print-length nil)
...
** messages-buffer
enable "q" to burry-window in messages-buffer-mode
...
(use-package compile
  ...
  (defcustom compilation-auto-jump-ignored-file-regexes nil
    "List of regular expressions for files to ignore in compilation buffers.
When a file name matches any of these regexes, `compilation--file-known-p'
will return nil even if the file can be found."
    :type '(repeat regexp)
    :group 'compilation)

  (setq compilation-auto-jump-ignored-file-regexes '("python*/site-packages"))

  (defun compilation--file-known-p ()
    "Say whether the file under point can be found.
Return nil if the file matches any regex in `compilation-ignored-file-regexes'."
    (when-let* ((msg (get-text-property (point) 'compilation-message))
		(loc (compilation--message->loc msg))
		(file-name (cadr (car (compilation--loc->file-struct loc))))
		(not-ignored (not (seq-some (lambda (regex)
                                              (string-match-p regex file-name))
                                            compilation-auto-jump-ignored-file-regexes)))
  ...
  (add-to-list 'compilation-error-regexp-alist-alist
               `(python-stacktrace
                 ,(rx (group "/" (+ (not (in "(\n")))) ; file path
                     ...
                     ")"
                     (? (+ (not (in ")"))))     ; optional function name
                     )
		 1 2))
  (add-to-list 'compilation-error-regexp-alist 'python-stacktrace)

  ;; /.../python/libs/streaming-parse/src/streaming_parse/json.py:362:19 - error: Argument of ...
  (add-to-list 'compilation-error-regexp-alist-alist
               `(pyright
                 ,(rx (group "/" (+ (not (in ":\n"))))  ; file path
                     ":"
                     ...
                     (group (+ digit))            ; column
                     " - "
                     (or (group "error")          ; group 4: error
                         (group "warning")        ; group 5: warning
                         (group "info"))          ; group 6: info
                     ": "
                     (* nonl))                    ; error message
                 1 2 nil                          ; FILE LINE COLUMN
                 (5 . 6)                          ; TYPE: warning=5, info=6, else error
                 1))                              ; HYPERLINK: highlight file only
  (add-to-list 'compilation-error-regexp-alist 'pyright)

  (add-to-list 'compilation-error-regexp-alist-alist
               `(ruff
                 ,(rx (or "  -->" "   -->")  ; Ruff uses arrows with indentation
                      " "
                      (group (+ (not (in ":\n"))))  ; file path
                      ":"
                      (group (+ digit))             ; line number
...
*** color
look into https://github.com/atomontage/xterm-color
...
#+begin_src emacs-lisp
(use-package emacs-zsh-history
  :demand
  :straight (:host github :repo "kawabata/emacs-zsh-history")
...
*** disable slow function on remote
...
(defun eshell/lcd (&optional directory)
  (if (file-remote-p default-directory)
      (with-parsed-tramp-file-name default-directory nil
        (message "%s" v)
        (message "%s" host)
        (message (format "/%s:%s@%s:%s" method user host (or directory "")))
        (message "%s" (tramp-make-tramp-file-name
                       (tramp-file-name-method v)
                       (tramp-file-name-user v)
                       (tramp-file-name-host v)
                       (or directory "")
                       (tramp-file-name-hop v)))
        (eshell/cd (format "/%s:%s@%s:%s" method user host (or directory ""))))
    (eshell/cd directory)))
#+end_src
...
(use-package auto-olivetti
  ...
  :custom
  (auto-olivetti-enabled-modes '(text-mode prog-mode Info-mode))
  :config
  (auto-olivetti-mode))
#+end_src
...
GET http://tt-agent-service.k.fal-dev.deepl.dev/api/task?userId=d3cb8bfd-acdd-4c22-8876-2971953f6d51
GET http://tt-agent-web.k.fal-dev.deepl.dev/api
#+end_src
...
<!-- Date: Wed, 09 Oct 2024 16:12:28 GMT -->
<!-- Content-Type: text/html -->
<!-- Content-Length: 190 -->
<!-- Connection: keep-alive -->
<!-- Request duration: 0.135381s -->
#+END_SRC
...
#+begin_src emacs-lisp
(setq browse-url-handlers '(("wiktionary" . eww-browse-url)))
#+end_src
...
(use-package webjumb
  :straight nil
  :config
  (add-to-list 'webjump-sites '("Wiktionary" . [simple-query "en.wiktionary.org" "https://en.wiktionary.org/wiki/" ""])))
#+end_src
...
(use-package gptel
  ...
  (general-def
    ...
    "C-<return>" 'gptel-send
    [remap evil-org-org-insert-heading-respect-content-below] 'maschm/gptel-send-org
    [remap org-insert-heading-respect-content-below] 'maschm/gptel-send-org
    )
  (my-menu-def
    "RET" (defun maschm/gptel-send-wim () (interactive) (gptel-send))
    "M-RET" 'maschm/gptel-dwim
    ...
    "G a" 'gptel-add
    "G C" 'gptel-add-file
    "G c" 'gptel-context-add-file ;; TODO toogle
    "G f" 'gptel-context-add-file
    "G +" (defun maschm/gptel-context-add-all-visible-buffers ()
            (interactive)
            (dolist (window (window-list))
              (with-current-buffer (window-buffer window)
                (gptel-add))))
    "G -" (defun maschm/gptel-context-remove-all ()
            (interactive)
            (gptel-context-remove-all nil))
    "G C" 'gptel-context-remove-all
    "G r" (list
    ...
    "+" 'gptel-add
    "." (list
         (defun maschm/gptel-dwim ()
           (interactive)
           (cond ((use-region-p)
                  (maschm/gptel-refactor))
                 ;; ((maschm/gptel-inside-context) (d))
                 (t (gptel-menu))))
         :which-key "gptel")
    )
  ...
  (gptel-org-convert-response nil) ;; transformation is a bit slow, TODO prompt the LLM to output org in the first place
  (gptel-org-branching-context nil)

  :config
  (require 'gptel-rewrite)
  (require 'gptel-org)
  (add-to-list 'warning-suppress-log-types '(gptel)) ;; dont display annoying gpel-model warning

  (setq gptel-expert-commands t) ; this is not a custom var TODAY

  ;; some keyremappings in gptel's transient interface
  ;; (with-eval-after-load 'transient
  ;;   (transient-suffix-put 'gptel-menu (kbd "-m") :key "M")
  ;;   (transient-suffix-put 'gptel-menu (kbd "-f") :key "f")
  ;;   (transient-suffix-put 'gptel-menu (kbd "-d") :key "D")
  ...
  ;;   (add-to-list 'gptel-org-ignore-elements 'comment)
  ;;   (add-to-list 'gptel-org-ignore-elements 'comment-block))

  ;; maybe fixes font-lock
  (add-hook 'gptel-post-response-functions #'font-lock-fontify-region)

  (defun maschm/gptel-send-org ()
    (interactive)
  ...
  (org-mode-hook . ((setq-local gptel--system-message 'maschm/build-org-system-message)))
  (gptel-mode-hook . ((general-def
                        :keymaps 'override
                        "C-<return>" 'maschm/gptel-send-org)
                      (evil-insert-state))))
#+end_src
...
#+begin_src emacs-lisp
(defun maschm/build-org-system-message ()
  "You are a large language model living in Emacs and a helpful assistant.

By default respond use the Org-mode markup language, because your response will be directly copying it into an Emacs buffer. This means:
- Use asterisks for headings (** for level 1, *** for level 2, etc.)
- Single star 0th level headings are reseved for the Editor
- Format text with *bold*, /italic/, _underlined_, and =code=
- Create lists with \"-\" or numbered lists with \"1.\" format
- Format links as [[url][description]]
- Use |table|syntax| for tables with |---+---| dividers
- Wrap code blocks in #+BEGIN_SRC language and #+END_SRC

Respond concisely.")

(with-eval-after-load 'gptel-mode
  (setf (alist-get 'org-mode gptel-directives) 'maschm/build-org-system-message))
#+end_src
...
(defun maschm/build-prog-message ()
  (let ((lang-name (major-mode-to-language-name))
        (script-syntax (major-mode-to-markdown-syntax)))
    (concat
"You are a careful expert " lang-name " programming assistant integrated directly into the Emacs text editor.
You're running as part of the user's Emacs environment and can provide solutions that leverage Emacs functionality.

Your responses should adapt to the type of input you receive:

1. When the user message contains mostly valid " lang-name " code with comments, questions, TODOs or pseudo code:
   - Modify the code to address the user's requests
   - Only output valid " lang-name " code
   - Do not use markdown code blocks or backticks
   - Maintain the existing code style and conventions

2. When the user provides a mix of code examples and natural language input:
   - Provide a comprehensive explanation
   - Include well-documented " lang-name " examples
   - Use markdown syntax with =" script-syntax " code blocks"
   (when (eq major-mode 'emacs-lisp-mode)
     "\n   - Where appropriate, include additional Emacs Lisp code to improve editor integration")

"\n\nAdditional guidance:
- When fixing errors, explain the underlying issue briefly before providing the solution
- If multiple approaches exist, briefly mention alternatives with their trade-offs
- Follow best practices and modern " lang-name " conventions
- If you need more context to provide a complete solution, ask clarifying questions

Always prioritize correctness, clarity, and maintainability in your code responses.")))
...
If MODE is nil, use the current buffer's major mode."
  (let ((mode (or mode major-mode)))
    (cond
     ...
     ((provided-mode-derived-p mode 'sh-mode) "sh script")
     ((provided-mode-derived-p mode 'yaml-mode) "YAML")
     ((provided-mode-derived-p mode 'json-mode) "JSON")
     ((provided-mode-derived-p mode 'toml-mode) "TOML")
     ((provided-mode-derived-p mode 'graphql-mode) "GraphQL")
     ((provided-mode-derived-p mode 'cmake-mode) "CMake")
     (t (let ((name (symbol-name mode)))
          (capitalize (replace-regexp-in-string "-mode$\\|-ts-mode$\\|-base-mode$" "" name)))))))
...
If MODE is nil, use the current buffer's major mode."
  (let ((mode (or mode major-mode)))
    (cond
     ...
     ((provided-mode-derived-p mode 'sh-mode) "bash")
     (t (replace-regexp-in-string "-mode$\\|-ts-mode$\\|-base-mode$" ""
                                  (symbol-name mode))))))
#+end_src
...
#+begin_src emacs-lisp
(setq maschm/ukrainian-prompt
      "You are a professional Ukrainian language tutor specialized in teaching Ukrainian to a German student inside.

,* Student Profile
- Language Level: Approximately A2 in Ukrainian
- Native Language: German
- Fluent Language: English
- Basic Language Knowledge: Spanish and French
- Script Proficiency: Fluent in Cyrillic script

,* Communication Guidelines
1) Language Output:
  - Use only Ukrainian Cyrillic script
  - Avoid transliteration or Latin character representations
  - Provide trilingual translations: Ukrainian/German/English

2) Linguistic Approach:
  - Highlight linguistic connections between Ukrainian, German, and English
  - Emphasize cognates and grammatical similarities

,* Error Handling
- Always correct mistakes in Ukrainian
- Offer constructive explanations

,* Output Format
a) When processing lists like inputs:
  - direct answers without extra output
  - follow the users input format in your answer
or
b) When processing long form text or questions, organize responses with:
  - Vocabulary Section with a focus on etymology and nuances
  - Grammar Explanation with comparative analysis
  - Practice Exercises without solutions
")

(with-eval-after-load 'gptel
  (setf (alist-get 'ukrainian gptel-directives) maschm/ukrainian-prompt))
#+end_src
...
(with-eval-after-load 'gptel-mode
  ...
  (gptel-make-tool
   :name "change_region_in_buffer"
   :function (lambda (buffer start end content)
               (with-current-buffer (get-buffer buffer)
                 (save-excursion
                   (goto-char start)
                   (delete-region start end)
                   (insert content))
                 (format "Changed region in buffer '%s' from %d to %d" buffer start end)))
   :description "Change a region in a specified buffer with the given content"
   :args (list '(:name "buffer"
                       :type string
                       :description "The name of the buffer to operate on")
               '(:name "start"
                       :type integer
                       :description "The start position of the region to change")
               '(:name "end"
                       :type integer
                       :description "The end position of the region to change")
               '(:name "content"
                       :type string
                       :description "The content to replace the region with"))
   :category "buffer")
   ...
   :function (lambda (buffer content)
               (with-current-buffer (get-buffer buffer)
                 (save-excursion
                   (goto-char (point-max))
                   (insert content))))
   :description "Append content to the end of a specified buffer"
   :args (list '(:name "buffer"
                       :type string
                       :description "The name of the buffer to operate on")
               '(:name "content"
                       :type string
                       :description "The content to append to the buffer"))
   :category "buffer")

  )

;; write a tool to replace a
#+end_src
...
(use-package claude-code-ide
  ...
  :general
  (my-menu-def
    "A" 'claude-code-ide-menu) ; Set your favorite keybinding
  :config
...
(use-package breadcrumb
  :hook (json-ts-mode . breadcrumb-mode))
...
(use-package python
  ...
  (my-leader-def
    ...
    "d" 'pdb)
  (general-def
    :keymaps '(python-base-mode-map python-mode-map python-ts-mode-map)
    "C-c C-p" 'maschm/run-python)
  :config
  ...
  (setq python-shell-process-environment  '("TERM=xterm-256color"))
  ...
  (defun maschm/python-shell-calculate-dedicated ()
    (if python-shell-dedicated python-shell-dedicated
      (if (project-current) 'project 'buffer)))

  (advice-add 'python-shell-send-buffer :before
              (defun advice:before:python-shell-send-buffer (&rest _)
                (save-excursion
                  (run-python (python-shell-calculate-command) (maschm/python-shell-calculate-dedicated) nil)
                  ;; (while (not python-shell--first-prompt-received)
                  ;;   (sit-for 0.1))
                  )))

  (defun maschm/run-python (&optional cmd dedicated show)
    (interactive
     (if current-prefix-arg
         (list
       ...
       (list (python-shell-calculate-command)
             (maschm/python-shell-calculate-dedicated)
             t)))
    (run-python cmd dedicated show))

  (advice-add 'python-shell-send-buffer :after 'python-shell-switch-to-shell)
...
(use-package python-pytest
  ...
  :config
  (inheritenv-add-advice 'python-pytest--run-command))
...
(add-hook 'python-pytest-setup-hook
           (lambda ()
...
(add-hook 'sql-interactive-mode-hook
          (defun maschm/sql-interactive-mode-hook ()
...
(use-package typescript-mode
  ...
  :local
  (typescript-ts-base-mode-hook . ((require 'lsp-javascript))))
#+end_src
...
My Ukrainian input method maps German phonetics to Ukrainian Cyrillic letters.
a) The necessity and desire for russian Cyrillic letters are not experienced by the author.
b) I have neither need nor desire for russian Cyrillic letters.
claudified: The two sentences convey the same meaning but differ significantly in style:


#+begin_src emacs-lisp
(quail-define-package
 "cyrillic-my-ukrainian" "Cyrillic[UK]" "🇺🇦"
 t "My Ukrainian layout." nil t t t t nil nil nil nil nil t)
...
(defun copy-current-project-path-as-kill ()
  "Copy the current file-name into the kill ring."
  (interactive)
  (let* ((root (project-root (project-current)))
...
** dir-locals
#+BEGIN_SRC emacs-lisp
(defun maschm/dir-locals-find ()
  (if-let* ((current-dir-locals-dir (locate-dominating-file default-directory #'dir-locals--all-files))
            (current-dir-locals-file (file-name-concat current-dir-locals-dir dir-locals-file)))
      current-dir-locals-file))

(defun maschm/dir-locals-open ()
  (interactive)
  (if-let* (current-dir-locals-file (maschm/dir-locals-find))
      (find-file current-dir-locals-file)))

;; not working
(defun dir-locals-read-from-parent-dirs ()
  "Apply all .dir-locals.el files found in parent directories."
  (let ((current-dir (directory-file-name default-directory))
        (dirs '()))
    (while (not (string= current-dir "/"))
      (setq current-dir (directory-file-name (file-name-directory current-dir)))
      (push current-dir dirs))
    (dolist (dir dirs)
...
#+begin_src emacs-lisp
(defun flatten-annotated-alist (lst)
  "Flatten a list of (ITEM . (ANNOTATIONS...)) to a list of (ITEM . ANNOTATION) for each annotation."
  (cl-loop for (item . annotations) in lst
           append (cl-loop for annotation in annotations
                           collect (cons item annotation))))
#+end_src
